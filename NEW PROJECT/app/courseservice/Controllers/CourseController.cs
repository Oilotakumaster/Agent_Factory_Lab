...
namespace CourseService.Controllers
{
    ...
    public class CourseController : ControllerBase
    {
        ...
        [HttpGet("getSecureVideoUrl/{courseId}/{userId}")]
        public IActionResult GetSecureVideoUrl(string courseId, string userId)
        {
            try
            {
                // 1. 設定 Dacast API Key (建議未來放進 Azure 設定檔，目前先直接放入)
                string dacastApiKey = "17770147498545eNe7EuuAmx8vtmvqd5O7tDOblZad";

                // 2. 決定 5 分鐘後的到期時間 (Unix Timestamp in seconds)
                long expires = DateTimeOffset.UtcNow.ToUnixTimeSeconds() + (5 * 60);

                // 3. 根據課程 ID 對應實際影片
                string dacastVideoId = "e65adbfa-b579-e2bb-919a-170b0d562a65/91c12faf-0dc4-4b50-9269-019d05d4654b";
                if (courseId == "1") dacastVideoId = "e65adbfa-b579-e2bb-919a-170b0d562a65/91c12faf-0dc4-4b50-9269-019d05d4654b";
                else if (courseId == "4") dacastVideoId = "e65adbfa-b579-e2bb-919a-170b0d562a65/91132b76-a6d4-4a4f-8146-cd04439ae881";
                else if (courseId == "5") dacastVideoId = "e65adbfa-b579-e2bb-919a-170b0d562a65/2a80f859-e62c-4e0a-9a89-543dbd3668fd";

                // 4. 準備加密的內容
                string messageToSign = $"{dacastVideoId}:{userId}:{expires}";

                // 5. 進行 HMAC-SHA256 簽章
                byte[] keyByte = Encoding.UTF8.GetBytes(dacastApiKey);
                ...
                // 6. 組裝安全票券網址
                string secureUrl = $"https://iframe.dacast.com/vod/{dacastVideoId}?token={signature}&expires={expires}";

                // 回傳 JSON
                var result = new { secureUrl = secureUrl };
                return Ok(result);
            }
            catch (Exception ex)
            {
